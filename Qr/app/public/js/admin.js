(function () {
  ...
  // 화면이 기다린 시간과 「누른 것 하나」는 public/js/clientTiming.js 가
  // 잰다 — 손님 화면(order.js)과 같은 코드를 쓴다. 여기서 감싸는 것은 그
  // 위층이라, 아래의 캐시로 끝난 요청은 그쪽까지 내려가지 않는다. 네트워크에
  // 안 나갔으니 그게 맞다.
  ...
  const ADMIN_I18N = {
    ko: {
      ...
      testBannerOther: "테스터 모드가 켜져 있어요 — 이 기기는 평소 그대로입니다. 여기서 넣는 주문은 진짜로 남고, 테스트 주문은 이 화면에 안 보입니다",
      ...
      diagSpeedHint: "어느 동작이 느린지 서버가 스스로 재서 쌓아둔 기록이에요. 느린 요청과 처음 뜨는 순간은 전부, 빠른 요청은 표본만 남습니다. 14일이 지나면 자동으로 지워져요.",
      diagRangeLabel: "기간",
      diagReloadBtn: "불러오기",
      diagDownloadBtn: "⬇ 기록 파일로 받기",
      diagLinkHintText: "받기가 안 되면 이 주소를 브라우저에서 열어주세요.",
    ...
    zh: {
      ...
      testBannerOther: "測試模式已開啟 — 這台裝置照常運作。在這裡送出的訂單會真實保留，測試訂單不會顯示在這個畫面",
      testBannerNobody: "測試模式已開啟，但沒有任何裝置參與中。目前沒有人在測試。",
      testBannerEndBtn: "結束",
      settingsCatDiag: "診斷 · 速度",
      settingsCatDiagSub: "找出卡住的地方 · 下載紀錄",
      diagSpeedTitle: "速度紀錄",
      diagSpeedHint: "伺服器自己量測並累積的紀錄，可以看出哪個動作比較慢。慢的請求和剛啟動的那一刻全部保留，快的請求只留樣本。超過 14 天會自動刪除。",
      diagRangeLabel: "期間",
      diagReloadBtn: "載入",
      diagDownloadBtn: "⬇ 下載紀錄檔",
      diagLinkHintText: "如果無法下載，請在瀏覽器開啟這個網址。",
