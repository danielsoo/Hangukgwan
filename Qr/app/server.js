...
// 지금 돌고 있는 배포의 지문을 모든 응답에 적어 보낸다.
//
// 2026-09-14 사장님: "부모님이 대만에 있어서 언제든 내가 수정해서 apk 를 줄
// 수 있는데 엄빠가 내일 한국으로 돌아가."
//
// 가게 태블릿은 주문판을 하루 종일 켜둔다. 껐다 켜는 일이 거의 없다. 그런데
// 새 배포가 나가도 그 화면은 **어제 받은 자바스크립트를 계속 쓴다** — 누가
// 새로고침을 누르기 전까지 영영. 지금까지는 "새로고침 한 번만 눌러줘"로
// 넘겼지만, 옆에 아무도 없으면 그게 안 된다. 고쳐도 안 닿는 것은 안 고친
// 것과 같다.
//
// 지문은 이미 있다(src/assetVersion.js — public/js, public/css 내용에서 뽑는다).
// 요청을 더 만들지 않는다. 주문판이 어차피 30초마다 서버를 찌르므로, 그
// 답에 한 줄 얹어 보내면 화면이 알아챈다. 새로고침할지는 화면이 정한다
// (public/js/clientTiming.js — 바쁠 때는 안 한다).
app.use((req, res, next) => {
  try {
    res.set("X-App-Version", require("./src/assetVersion").assetVersion());
  } catch (e) {
    // 지문을 못 구해도 요청은 그대로 간다.
  }
  next();
});
