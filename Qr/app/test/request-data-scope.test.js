...
// 주문 질의 두 개와 콜드 스타트용 DB 작업을 똑같이 기다리고 있었다.
const fs = require("fs");
...
(async () => {
  ...
  out.push("\n[테이블 주소도 필요한 동작만]");
  const scoped = [
    ["자리 목록", { method: "GET", originalUrl: "/api/tables" }, true],
    ["인원·최소금액 조회", { method: "GET", originalUrl: "/api/tables/7/party-size" }, true],
    ["테이블 삭제 안전 확인", { method: "DELETE", originalUrl: "/api/tables/7" }, true],
