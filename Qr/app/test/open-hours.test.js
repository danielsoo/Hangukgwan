...
out.push("[이 가게가 실제로 쓰는 시각]");
// 사장님(2026-09-10): "오전장사 11:00~13:30 / 저녁장사 17:00~20:30.
// 문닫기 30분 전이라 좌석에 앉아 있지만 마감시간이기때문에 qr 주문 불가."
//
// 손님에게 보여주는 영업시간(11:00-14:00, 17:00-21:00)과 30분씩 다르다.
// 문구에서 읽어와 지어냈으면 20:59 까지 받았을 것이다 — 이 30분이
// 「보여주는 문구」와 「막는 규칙」을 나눠둔 이유 그 자체다.
{
  const { DEFAULT_ORDER_RANGES } = require("../src/migrations/2026-09-10-order-hours");
  const cfg = { order_hours: oh.normalize({ enabled: 1, ranges: DEFAULT_ORDER_RANGES }) };
  const at = (hhmm) => oh.isOpenNow(cfg, `2026-09-10 ${hhmm}:00`);
  check("점심 11:00 받는다", at("11:00"));
  check("점심 13:29 받는다", at("13:29"));
  check("점심 13:30 부터 안 받는다", !at("13:30"));
  check("영업은 하지만 13:45 에는 안 받는다", !at("13:45"));
  check("저녁 17:00 받는다", at("17:00"));
  check("저녁 20:29 받는다", at("20:29"));
  check("저녁 20:30 부터 안 받는다", !at("20:30"));
  check("문 닫기 전 20:50 에도 안 받는다", !at("20:50"));
}
