...
import CountryCodeInput from '../../language/CountryCodeInput';
...
function Signup() {
  ...
  // 회원가입 성공 시 표시할 이름: 영어(en)인 경우에는 firstName만, 그 외는 lastName + firstName (공백 없이)
  const displayName =
    i18n.language === 'en' ? firstName : `${lastName}${firstName}`;

  if (signupSuccess) {
    return (
      <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 3 }}>
        <SignupSuccess userName={displayName} />
      </Box>
    );
  }
